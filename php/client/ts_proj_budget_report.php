...
echo "<ul>";
	echo "<li>";
	$userstr	=	"?".base64_encode($userinfo."&proj=y&casethismth=y");
	echo "<a href=\"$PHP_SELF$userstr\"><b>Current</b></a>&nbsp;&nbsp;";
	$userstr	=	"?".base64_encode($userinfo."&proj=y&caselastmth=y");
	echo "<a href=\"$PHP_SELF$userstr\"><b>Previous</b></a>";
	echo " <b>Month</b></li>";
	
	echo "<li>";
	$userstr	=	"?".base64_encode($userinfo."&proj=y&caseyear=y");
	echo "<a href=\"$PHP_SELF$userstr\"><b>Year to Date</b></a>&nbsp;&nbsp;";
	$userstr	=	"?".base64_encode($userinfo."&proj=y&caselastyear=y");
	echo "<a href=\"$PHP_SELF$userstr\"><b>Last Year</b></a>";
	echo "</li>";
	
	if ($priv == "00") {
		$userstr	=	"?".base64_encode($userinfo."&proj=y&caseany=y");
		echo "<li><a href=\"$PHP_SELF$userstr\"><b>Any Period</b></a></li>";
	}
echo "</ul>";
	...
	if ($priv == "00" || $email_name == "olc") {//
		...
		echo "<ul>";
		$userstr	=	"?".base64_encode($userinfo."&rsc=y&casethismth=y");
		echo "<li><a href=\"$PHP_SELF$userstr\"><b>This Month</b></a>&nbsp;";
		$userstr	=	"?".base64_encode($userinfo."&rsc=y&caselastmth=y");
		echo "<a href=\"$PHP_SELF$userstr\"><b>Last Month</b></a>";
		echo "</li>";
		$userstr	=	"?".base64_encode($userinfo."&rsc=y&caseyear=y");
		echo "<li><a href=\"$PHP_SELF$userstr\"><b>Year to Date</b></a>&nbsp;&nbsp;";
		$userstr	=	"?".base64_encode($userinfo."&rsc=y&caselastyear=y");
		echo "<a href=\"$PHP_SELF$userstr\"><b>Last Year</b></a>";
		echo "</li>";	
		if ($priv == "00") {
...
//Prepare date range
	if ($casethismth) {
		...
		$nodays = finddaysinFeb($month, $year);
		$datefrom = "$year-$month-01";
		$dateto = "$year-$month-$nodays";
	} elseif ($caseyear) {
		$year = date("Y");
		if (date("m") < 7) {
			 $year -= 1;
...
#	PROJECT REPORT
	...
	if ($casethismth) {
		$title = "$rptstr for $thismonth ($datefrom to $dateto)";
	} elseif ($caselastmth) {
		$title = "$rptstr for Last Month ($datefrom to $dateto)";
	} elseif ($caseyear) {
		$title = "$rptstr for Year to Date ($datefrom to $dateto)";
	} elseif ($caselastyear) {
		$title = "$rptstr for Last Year ($datefrom to $dateto)";
	} elseif ($caseany) {
...
if ($proj) {
   ...
   while (list($bgtfileidx, $projcode_id, $description, $client, $begin_date, $end_date) = mysql_fetch_array($result)) {
	...
	if ($nobdgtctr) {
		if ($nobdgtctr > 1) {
			echo "<h4><font color=#0000ff>$nobdgtctr project budget data have found for this period.</font></h4><p>";
			fputs($csvfh, "\n\n\n$nobdgtctr project budget data have found for this period.\n");
		} else {
			echo "<h4><font color=#0000ff>Found $nobdgtctr project budget data for this period.</font></h4><p>";
			fputs($csvfh, "\n\n\n$nobdgtctr project budget data has found for this period.\n");
		}

		echo "<table border=1 $tabbg>";
		echo "<tr><th>No</th><th>Project</th><th>Total Hours</th><th>Date Start</th><th>Date End</th><th>Description</th></tr>";
		fputs($csvfh, "\nNo,Project,Total Hours,Date Start,Date End,Description\n");
		$ctr = " align=middle";
		$right = " align=right";
		for ($i=0; $i<$nobdgtctr; $i++) {
			...
			echo "<tr$rowbg1><th>$No</th><td><b>$Project</b></td><td$ctr><b>
				&nbsp;</b></td><td$ctr>$DateStart</td><td$ctr>$DateEnd</td><td><b>$Description</b></td></tr>";
			...
			//find tasks
			$sql = "SELECT m_stasks, tasks, date_start, date_end, hours
			...
			if (!$no) {
				...
				$rowctr=0;
 				//echo "$no <br>$sql<br>";
				while (list($m_stasks, $tasks, $date_start, $date_end, $hours) = mysql_fetch_array($result)) {
					...
					echo "<td$ctr>$hours</td><td$ctr>$date_start</td><td$ctr>$date_end</td>$tmp$tasks$tmp2</td></tr>";
				...
				if ($mstonectr) {
					for ($ii=0; $ii<$mstonectr; $ii++) {
						...
						echo "<tr><td colspan=3 align=middle><b>Milestone</b></td><td align=middle>$dstr</td>
		...
		echo "<h3><font color=#ff0000>No project budget data found for this period.</font></h3><p>";
		fputs($csvfh, "\n\n\nNo project budget data found for this period.\n");
	}
	fclose($csvfh);
} elseif ($rsc) {	//resource allocation
