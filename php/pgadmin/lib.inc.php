...
/* $Id: lib.inc.php,v 1.37 2001/04/23 19:50:25 dwilson Exp $ */
...
if ($server == 0) {
	...
	if ($cfgServer['adv_auth']) {	
		...
		if (empty($PHP_PGADMIN_USER) || empty($PHP_PGADMIN_PW)) {
			...
			if ($dbh = pg_connect($conn_str)) {
				...
				$rs = pg_exec($dbh, pre_query($sql_get_auth)) or pg_die(pg_errormessage(), $sql_get_auth, __FILE__, __LINE__);
	...
	$sql_get_userperm = "SELECT usecreatedb FROM pg_user WHERE usename = '$cfgServer[stduser]'";
	...
	if (!empty($PHP_PGADMIN_USER)) {
		$strSuperUser = $PHP_PGADMIN_USER;
	} else {
		$strSuperUser = $cfgServer[stduser];
	}
	$qrIsSuper = "SELECT usesuper FROM pg_user WHERE usename = '$strSuperUser' AND usesuper = 't'";
...
function display_table ($dt_result) {
	...
	global $cfgQuotes, $cfgMaxPages, $strMore, $cfgMaxText;
