<?php
/* $Id: db_details.php,v 1.16 2001/03/16 18:59:29 dwilson Exp $ */
	...
	switch($rel_type) {		
				...
				echo "
					<table border=$cfgBorder>
						<tr bgcolor=lightgrey>
							<th>$strView</th>
							<th align=center colspan=5>$strAction</th>
							<th>$strRecords</th>
						</tr>
					";
		...
		case "sequence":
			...
			if ($num_seq == 0) {
				...
				$i = 0;
				echo "<table border=$cfgBorder><tr bgcolor=lightgrey><th colspan=3>$strSequences</th></tr>\n";
...
<?php echo $strRunSQLQuery.$db." ".show_docu("sql-select.htm");?>:<br>
<textarea name="sql_query" cols="40" rows="3" wrap="VIRTUAL" style="width: <?php echo $cfgMaxInputsize;?>"><?php echo stripslashes($sql_query); ?></textarea>
...
<li><a href="sql.php?server=<?php echo $server;?>&db=template1&sql_query=<?php echo urlencode("DROP DATABASE $cfgQuotes$db$cfgQuotes");?>&zero_rows=<?php echo urlencode($strDatabase." ".$db." ".$strHasBeenDropped);?>&goto=main.php&reload=true"><?php echo $strDropDB." ".$db;?></a>
<br><br>
